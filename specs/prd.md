...
*   **Logic**:
    ...
    *   Users can update these at any time. Updates reflect typically on *display* logic (shadow text), but do not change the recorded historical face-value of debts.
...
*   **Balances**:
    ...
    *   Balances are calculated *per currency* first.
...
*   *Constraint: Unique per user+currency*
...
*   `type` (enum: 'DEBT', 'REPAYMENT') — *Or just use signed amounts*
*   `amount` (decimal)
